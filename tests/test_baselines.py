...
class BaselineFeatureTests(unittest.TestCase):
    ...
    def test_feature_shape_and_relative_pose(self) -> None:
        ...
        features = train_baselines.record_to_features(
            record
        )
        ...
        feature_matrix = features.reshape(
            120,
            7,
        )

        np.testing.assert_allclose(
            feature_matrix[0, 0:3],
            np.zeros(3),
            atol=1e-10,
        )
    ...
    def test_metadata_does_not_change_features(self) -> None:
        ...
        first_features = (
            train_baselines.record_to_features(
                first_record
            )
        )
    ...
    def test_build_datasets_keeps_splits_separate(self) -> None:
        ...
        datasets = train_baselines.build_datasets(
            records
        )

        self.assertEqual(
            datasets["train"]["features"].shape,
            (5, 840),
        )

        self.assertEqual(
            datasets["validation"]["features"].shape,
            (5, 840),
        )

        self.assertEqual(
            datasets["test"]["features"].shape,
            (5, 840),
        )

        self.assertEqual(
            set(datasets["train"]["labels"]),
            set(train_baselines.LABELS),
        )
